...
Add a long random secret in Railway. Login uses Noyra-issued, single-use magic
links and does not expose public registration.

```env
NOYRA_API_KEY="<your-long-random-noyra-api-key>"
```

Create accounts with `POST /api/noyra/accounts`, or issue a new link for an
existing account with `POST /api/noyra/login-links`. Both require an
`Authorization: Bearer <NOYRA_API_KEY>` header.
