...
<Tip>
  ...
  <Step title="Account provisioning key">
useSend uses one-time magic links supplied by Noyra and does not expose public
registration or send login emails. Create a long random secret for the Noyra
account endpoints.
...
Create accounts with `POST /api/noyra/accounts`, or issue a fresh link for an
existing account with `POST /api/noyra/login-links`. Both endpoints require an
`Authorization: Bearer <NOYRA_API_KEY>` header and return a single-use magic
link that expires after 10 minutes.
